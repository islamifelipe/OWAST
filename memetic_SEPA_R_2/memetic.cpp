...
int contCrossovers= 0; // quantidade de crossovers executados
int contMutacoes= 0; // quantidade de mutaçoes executadas
int contSucessoCrossovers= 0; // quantidade de crossovers com sucesso (individuos melhores que os pais)
float sumSucessoCrossvers = 0;
int contSucessoMutacoes= 0; // quantidade de mutaçoes com sucesso (individuos melhores que o anterior sem mutaçao)
float sumSucessoMutacoes = 0;
int contSucessoSA= 0; // quantidade de vezes em que o SA retornou uma soluçao melhor
int contQuantCalculouFitness= 0; // guarda a quantidade de vezes em que a funçao calculaOwa foi invocada
int sumSucessoSA = 0;
int quantSAinvocado= 0; // quantidade de vezes em que o SA foi invocado
...
SolucaoEdgeSet * memetic(){
	...
	gerarPopulacao3(populacao, vetoresDirecoes); // gera populaçao inicial
	*otimo = *populacao[0];
	int p1,p2,p3,p4;
	...
	for (int i=0; i<QUANTGERACOES; i++){ // para cada geraçao...
		...
		if (contSemMudanca==5){
			...
			for (int i=0; i<TAMANHOPOPULACAO/2; i++) {
				int idfjifj  = IRandom(0,TAMANHOPOPULACAO-1);
				rmcPrim(*filho, w);
				filho->calculaOwa(w);
		...
		for (int j=0; j<TAMANHOPOPULACAO; j++){ // deve-se criar TAMANHOPOPULACAO novos individuos
			...
			if (p<TAXADECRUZAMENTO){
				...
				filho->calculaOwa(w);
				if (filho->getOWA()<populacao[pai]->getOWA() || filho->getOWA()<populacao[mae]->getOWA()) {
					contSucessoCrossovers++;
					float maaaxxx = populacao[pai]->getOWA() - filho->getOWA();
					if (populacao[mae]->getOWA() - filho->getOWA() > maaaxxx ) maaaxxx = populacao[mae]->getOWA() - filho->getOWA();
					sumSucessoCrossvers += maaaxxx; 
				}
			} else {
				filho->doRandomWalk();
				filho->calculaOwa(w);
			...
			if (p<TAXADEMUTACAO){
				...
				if (novaPop[j]->getOWA() < filho->getOWA()) {
					contSucessoMutacoes++;
					sumSucessoMutacoes += filho->getOWA() - novaPop[j]->getOWA();
				}
			} else{
				filho->calculaOwa(w);
				*novaPop[j] = *filho;
			...
			SA(*novaPop[j]);
			if (novaPop[j]->getOWA()<fitsss) {
				contSucessoSA++;
				//cout<<novaPop[j]->getOWA()<<" ==> "<<fitsss<<endl;
				sumSucessoSA += fitsss - novaPop[j]->getOWA();
			}
		}
...
int main(int argc, char *argv[]){
	...
	fprintf(tempofile,"%.2lf\n", (double) (tempoDepois.tms_utime - tempoAntes.tms_utime) / 100.0 );
   	...
   	cout<<"Média de melhoria das crossovers = "<<(float)sumSucessoCrossvers/contCrossovers<<endl;
   	...
   	cout<<"Média de melhoria das mutaçoes = "<<(float)sumSucessoMutacoes/contMutacoes<<endl;
   	...
   	cout<<"Média de melhoria do SA = "<<(float)sumSucessoSA/quantSAinvocado<<endl;
