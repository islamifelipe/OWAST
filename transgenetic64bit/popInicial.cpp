...
void gerarPopulacao1(SolucaoEdgeSet *populacao[TAMANHOPOPULACAO]){
	...
	int iiiiii, cont;
	for (iiiiii=0, cont = 0; cont<(int)PORCENT_RANDOM*TAMANHOPOPULACAO/100 && iiiiii<TAMANHOPOPULACAO; iiiiii++, cont++){
		populacao[iiiiii]->doRandomWalk();
		populacao[iiiiii]->calculaOwa(w);
	}
	cout<<"================== rmcPrim com k-centrum ================== "<<endl;
	for (cont = 0; cont<(int)PORCENT_KCENTRUM*TAMANHOPOPULACAO/100 && iiiiii<TAMANHOPOPULACAO; iiiiii++, cont++){
		int k = IRandom(1,NUMOBJETIVOS);
		double lambda[NUMOBJETIVOS];
		for (int ll=0; ll<k; ll++)lambda[ll] = 1.0/k;
		for (int ll=k; ll<NUMOBJETIVOS; ll++) lambda[ll] = 0;
		// rmcPrim(*populacao[iiiiii], lambda, rg);
		rmcPrim(*populacao[iiiiii], lambda);
		populacao[iiiiii]->calculaOwa(w);
	}
	...
	int lim = (int) NUMOBJETIVOS/2-1;
	if (lim==0) lim = 1;
	for (cont = 0; cont<(int)PORCENT_KTRIMENT*TAMANHOPOPULACAO/100 && iiiiii<TAMANHOPOPULACAO; iiiiii++, cont++){
	...
	double alfa[6] = {0.2, 0.3, 0.4, 0.6, 0.7, 0.8};
	for (cont = 0; cont<(int)PORCENT_HURWICZ*TAMANHOPOPULACAO/100 && iiiiii<TAMANHOPOPULACAO; iiiiii++, cont++){
		int index = IRandom(0,5);
		double lambda[NUMOBJETIVOS];
		lambda[0] = alfa[index];
		lambda[NUMOBJETIVOS-1] = 1.0 - alfa[index];
...
void gerarPopulacao2(SolucaoEdgeSet *populacao[TAMANHOPOPULACAO], double vetoresDirecoes[NUMDIRECOES][NUMOBJETIVOS]){	
	...
	for (int cont = 0; cont<TAMANHOPOPULACAO; cont++){
		...
		rmcPrim(*populacao[cont], lambda);
		populacao[cont]->calculaOwa(w);
	}

}

void gerarPopulacao3(SolucaoEdgeSet *populacao[TAMANHOPOPULACAO], double vetoresDirecoes[NUMDIRECOES][NUMOBJETIVOS]){
	...
	SolucaoEdgeSet *aux = new SolucaoEdgeSet(NUMEROVERTICES-1);
	int contRandom = 0, speaR = 0, trimed = 0, centrum=0, hur=0;
	int op = 0;
	for (int cont = 0; cont<TAMANHOPOPULACAO; cont++){
		double lambda[NUMOBJETIVOS];
		// int indfgf = rg.IRandom(0, NUMDIRECOES-1);
		int indfgf = IRandom(0, NUMDIRECOES-1);
		for (int ll=0; ll<NUMOBJETIVOS; ll++){
